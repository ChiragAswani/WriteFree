...
Now you can rename `src/App.css` to `src/App.scss` and update `src/LoginPage.js` to import `src/App.scss`.
...
4. Add `// @flow` to any files you want to type check (for example, to `src/LoginPage.js`).
...
To try it, delete all the code in `src/LoginPage.js` and replace it with any of the examples on its website. The [Basic Example](https://reacttraining.com/react-router/web/example/basic) is a good place to get started.
